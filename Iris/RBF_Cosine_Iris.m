...
%     I(k)=0;
    ...
    for i1=1:m1
        for i2=1:n1
            phi(1,i2)=(train_data(ind(i1),:)*c(i2,:)')/(norm(train_data(ind(i1),:))*norm(c(i2,:))+1e-50);
        end
...
for i1=1:m2
    for i2=1:n1
        phi(1,i2)=(test_data(i1,:)*c(i2,:)')/(norm(test_data(i1,:))*norm(c(i2,:))+1e-50);
    end
    y_test(i1)=w*phi(1,:)' + b;
end
...
save('Results\I1.mat','I','eta','f','y_test_final','Training_Accuracy','Testing_Accuracy','alpha_final');
