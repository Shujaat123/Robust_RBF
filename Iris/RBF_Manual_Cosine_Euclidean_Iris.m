...
%     I(k)=0;
    ...
    for i1=1:m1
        for i2=1:n1
            CD=(train_data(ind(i1),:)*c(i2,:)')/(norm(train_data(ind(i1),:))*norm(c(i2,:))+1e-50);
            ED=exp((-(norm(train_data(ind(i1),:)-c(i2,:))^2))/beeta^2);
...
save('Results\I3.mat','I','eta','f','y_test_final','Training_Accuracy','Testing_Accuracy','alpha_final');
