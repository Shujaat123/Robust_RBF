...
for i1=1:m2
    for i2=1:n1
            CD(1,i2)=(test_data(ind(i1),:)*c(i2,:)')/(norm(test_data(ind(i1),:))*norm(c(i2,:))+1e-50);
            ED(1,i2)=exp((-(norm(test_data(ind(i1),:)-c(i2,:))^2))/beeta^2);            
        end
        y_test(i1)=w(1,:)*CD(1,:)'+ w(2,:)*ED' +b;
end
y_test_final=y_test_final+y_test;
alpha_final=alpha_final+[norm(w(1,:)) norm(w(2,:))]./sum([norm(w(1,:)) norm(w(2,:))]);
Testing_Accuracy(run)=100*(1-(length(find((round(y_test)-class_test)~=0))/length(y_test)));
end
I=I./(runs*m1);
alpha_final=alpha_final./runs;
y_test_final=y_test_final./runs;

save('Results\I5.mat','I','eta','f','y_test_final','Training_Accuracy','Testing_Accuracy','alpha_final');
